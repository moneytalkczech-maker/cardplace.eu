...
import {
  ...
  ChevronRight, LogOut, ArrowLeft, User, Zap,
} from "lucide-react";
...
export default function AdminLayout({ children, title }: { children: React.ReactNode; title?: string }) {
  ...
  const [expandedGroups, setExpandedGroups] = useState<Record<string, boolean>>(() => {
    ...
    for (const group of NAV_GROUPS) {
      saved[group.label] = group.items.some((item) => location.pathname.startsWith(item.path));
    }
    return saved;
  });
  ...
  const currentLabel = BREADCRUMB_MAP[currentPath] || title || "Admin";
  ...
  return (
    <div className="flex min-h-screen bg-[#050A12]">
      ...
      {/* ─── Sidebar ─── */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-[#0B1220] border-r border-[rgba(0,200,255,0.08)] transform transition-transform duration-200 ${
      ...
      >
            ...
            return (
              <div key={group.label}>
                <button
                  ...
                  className={`flex items-center justify-between w-full px-3 py-2 rounded-lg text-xs font-heading font-bold uppercase tracking-wider transition-colors ${
                    hasActive ? "text-[#00C8FF]" : "text-gray-600 hover:text-gray-400"
                  }`}
                >
                  {group.label}
                    ...
                    {group.items.map((item) => {
                      ...
                      return (
                        <Link
                          key={item.path}
                          to={item.path}
                          className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-heading transition-all ${
                            active
                              ? "bg-[rgba(0,200,255,0.1)] text-[#00C8FF] font-semibold"
                              : "text-gray-400 hover:text-white hover:bg-[rgba(0,0,0,0.2)]"
                          }`}
                        >
        ...
        <div className="border-t border-[rgba(0,200,255,0.08)] p-3 space-y-1 flex-shrink-0">
          <Link
            to="/"
            className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-heading text-gray-400 hover:text-white hover:bg-[rgba(0,0,0,0.2)] transition-all"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 h-16 border-b border-[rgba(0,200,255,0.08)] bg-[#050A12]/90 backdrop-blur-lg flex-shrink-0">
          <div className="flex items-center justify-between h-full px-4 lg:px-6">
            {/* Levá část – hamburger + breadcrumbs */}
            ...
            {/* Pravá část – role badge + user info + logout */}
            <div className="flex items-center gap-3">
