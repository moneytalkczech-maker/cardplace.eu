...
export default function ToastContainer() {
  ...
  return (
    <div className="fixed bottom-20 md:bottom-4 right-4 z-[100] flex flex-col gap-2 max-w-sm pointer-events-none">
      {toasts.map((t) => {
        ...
        return (
          <div
            key={t.id}
            className={`pointer-events-auto flex items-start gap-3 px-4 py-3 rounded-xl border shadow-2xl backdrop-blur-md ${c.bg} ${c.border} animate-slide-up`}
          >
            <Icon className={`h-5 w-5 flex-shrink-0 mt-0.5 ${c.text}`} />
            <p className="text-sm text-white flex-1">{t.message}</p>
            <button onClick={() => remove(t.id)} className="btn-ghost p-0.5 -mr-1">
