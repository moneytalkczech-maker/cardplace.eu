...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(axiosErr.response?.data?.error || t("auth.loginError"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-[#050A12]">
      <div className="absolute inset-0">
        <div className="absolute top-0 left-1/4 w-[400px] h-[400px] bg-[#00C8FF] rounded-full opacity-[0.03] blur-[100px]" />
        <div className="absolute bottom-0 right-1/4 w-[300px] h-[300px] bg-[#FF0044] rounded-full opacity-[0.02] blur-[80px]" />
      </div>
      ...
      <div className="relative mx-auto max-w-sm px-4 py-20 w-full">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold font-heading">
            <span className="text-white">Vítej </span>
            <span className="text-gradient">zpět</span>
          </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end -mt-2">
            <Link href="/forgot-password" className="text-xs text-gray-500 hover:text-[#00C8FF] transition-colors">
          ...
          <a href="/api/auth/google" className="flex items-center justify-center gap-3 w-full px-4 py-3 rounded-xl border border-[rgba(0,200,255,0.15)] bg-[rgba(0,0,0,0.3)] hover:bg-[rgba(0,200,255,0.06)] transition-colors font-heading font-semibold text-sm text-white">
          ...
          <p className="text-center text-sm text-gray-500">
            {t("auth.noAccount")}{" "}
            <Link href="/register" className="text-[#00C8FF] hover:text-[#00C8FF]/80 font-heading font-semibold">{t("auth.signUp")}</Link>
          </p>
        </form>
      </div>
    </div>
